...
def quiz_home():
    if request.method == "POST":
        if request.form.get("link") == "rozpocznij-quiz":
            obj = [
                i.to_dict()
                for i in Question.query.filter_by(user_id=current_user.id).all()
            ]
...
def quiz_add():
    ...
    if (
        request.method == "POST"
        and Question.query.filter_by(question=request.data.decode()).first()
    ):
        question = Question.query.filter_by(question=request.data.decode()).first()
        answers = Answer.query.filter_by(question_id=question.id).first()
        db.session.delete(answers)
        db.session.delete(question)
        db.session.commit()
        obj = [
            i.to_dict() for i in Question.query.filter_by(user_id=current_user.id).all()
        ]
        session["current_user_questions"] = obj
        ...
        conditions = {
            "Treść pytania - to miejsce jest wymagane": question,
            ...
            "Jedna z podanych odpowiedzi musi być poprawna": correct_answer
            in [answer1, answer2, answer3, answer4],
        }
        ...
        if all(conditions.values()):
            question = Question(question=question, user_id=current_user.id)
...
def quiz_start():
    ...
    session["question"] = session["questions"][session["question_number"] - 1][
        "question"
    ]
